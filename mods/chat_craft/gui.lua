...
local players = {}
local registered_crafts = chat_craft.registered_crafts
...
minetest.register_on_mods_loaded(function()
	for item, def in pairs(minetest.registered_items) do
		for group, level in pairs(def.groups or {}) do
			if not chat_craft.group_icons[group] then chat_craft.group_icons[group] = item end
		end
	end
end)
...
function chat_craft.get_recipe_list_fs(player, width, scroll_index)
	local recipes = chat_craft.get_craftable_recipes(player)

	local f = {
		"box[0,0;" .. width .. ",4.75;#FFF0]",
		"box[" .. width + 0.25 .. ",0;0.5,4.75;#0000]",
		"image[0,0;" .. width .. ",4.75;[combine:" .. width * 16 .. "x" .. 4.75 * 16 .. ":0,0=fc_grid.png^[multiply:" .. chat_craft.grid_color .. ";]",
	}
...
local function item_image_fs(x_pos, y_pos, item, count)
	...
	if item:find("group") then
		...
		f[#f + 1] = "item_image[" .. x_pos .. "," .. y_pos .. ";1,1;" .. chat_craft.group_icons[groups[1]] .. "]"
...
function chat_craft.get_recipe_fs(width, recipe_index)
	local fs = {
		"box[0,0;" .. width .. ",1;#FFF0]",
		"box[0,1.5;" .. width .. ",3;#FFF0]",
		"image[0,0;" .. width .. ",1;[combine:" .. width * 16 .. "x" .. 1 * 16 .. ":0,0=fc_grid.png^[multiply:" .. chat_craft.grid_color .. ";]",
		"image[0,0;1,1;[combine:" .. 1 * 16 .. "x" .. 1 * 16 .. ":0,0=fc_grid.png^[multiply:" .. chat_craft.highlight_color .. ";]",
		"image[0,1.5;" .. width .. ",3;[combine:" .. width * 16 .. "x" .. 3 * 16 .. ":0,0=fc_grid.png^[multiply:" .. chat_craft.grid_color .. ";]",
	}
...
function chat_craft.get_craft_buttons_fs(player, recipe_index)
	local fs = {}
	local recipe = registered_crafts[recipe_index]
	if recipe then
		for i = 1, 4 do
			fs[#fs + 1] = "box[0," .. (i-1) * 0.75 .. ";0.5,0.5;#0000]"
			fs[#fs + 1] = "button[0," .. (i-1) * 0.75 .. ";0.5,0.5;craft_" .. chat_craft.craft_sizes[i] .. "_" .. recipe_index .. ";" .. chat_craft.craft_sizes[i] .. "]"
			fs[#fs + 1] = "tooltip[craft_" .. chat_craft.craft_sizes[i] .. "_" .. recipe_index .. ";" .. S("Craft @1", chat_craft.craft_sizes[i]) .. "]"
		end
		local max = chat_craft.get_craft_amount(recipe, chat_craft.inv_to_table(player:get_inventory()))
...
function chat_craft.get_chat_craft_formspec(player, width, scroll_index, recipe_index)
	...
	local fs = {
		...
		chat_craft.get_recipe_list_fs(player, list_width, scroll_index),

		"container[" .. list_width + 1 .. ",0]",
		chat_craft.get_recipe_fs(recipe_width, recipe_index),
		"container_end[]",

		"container[" .. button_x .. ",0]",
		chat_craft.get_craft_buttons_fs(player, recipe_index),
		"container_end[]",
	}
	return table.concat(fs)
end
...
function chat_craft.get_craft_invenory_formspec(player, scroll_index, recipe_index)
	local fs = {
		"formspec_version[6]",
		"size[13.25,12]",
		...
		chat_craft.get_chat_craft_formspec(player, 12.25, scroll_index, recipe_index),
...
function chat_craft.regsiter_formspec_callback(form_name, callback)
	minetest.register_on_player_receive_fields(function(player, formname, fields)
		...
		for k, v in pairs(fields) do
			...
			if craft_amount then
				...
				craft_index = tonumber(craft_index)
				chat_craft.craft(player, craft_index, craft_amount)
