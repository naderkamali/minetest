		...
		for item, def in pairs(minetest.registered_items) do
			...
			for i, recipe in pairs(recipes) do
				if recipe.method == "normal" then
					...
					-- if out.item:is_empty() then print("chat_craft: " .. item ) end
					local a_output = {}
					...
					if simplify_recipes then
						fc_recipe = chat_craft.simplify_recipe(fc_recipe)
					end

					chat_craft.register_craft(fc_recipe)
				end
			end
		end

		print("[MOD] 'chat_craft' recipes imported in " .. (minetest.get_us_time() - t1)/1000000 .. " s")
	end)
end
