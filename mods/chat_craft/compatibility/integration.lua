...
local S = chat_craft.translator
local p16 = 1/16
local gameid = Settings(minetest.get_worldpath()..DIR_DELIM..'world.mt'):get('gameid')

print("[chat_craft] gameid: " .. gameid)


-- Default integration, always accessible

-- show crafting formspec independently of players 'inventory_formspec'
function chat_craft.show_crafting_inventory(player, scroll_index, craft_index)
	local fs = chat_craft.get_craft_invenory_formspec(player, scroll_index, craft_index)
	minetest.show_formspec(player:get_player_name(), "chat_craft:crafting", fs)
end
chat_craft.regsiter_formspec_callback("chat_craft:crafting", chat_craft.show_crafting_inventory)

-- minetest.register_chatcommand("chat_craft", {
-- 	params = "",
-- 	description = "Show independent chat_craft crafting formspec.",
-- 	privs = {},
-- 	func = function(name, param)
-- 		local player = minetest.get_player_by_name(name)
-- 		chat_craft.show_crafting_inventory(player, nil, nil)
-- 	end,
-- })

-- make crafting a chat command
-- minetest.register_chatcommand("chat_craft", {
-- 	func = function(name, param)
-- 		local player = minetest.get_player_by_name(name)
		
-- 		minetest.debug("Crafting intiated by: " .. tostring(player))
...
-- 		-- Check if the crafting recipe exists
-- 		local recipes = chat_craft.get_craftable_recipes(player)
-- 		local recipe = recipes[param]
-- 		-- print_table(chat_craft.registered_crafts)
-- 		minetest.debug("Recipe name: " .. param)

-- 		if recipe then
-- 			chat_craft.craft(name, recipes[param], 1)
-- 		end
-- 	end,
-- })
...
if gameid == "cicrev" then
	...
	local function get_cicrev_sfinv_page(player, scroll_index, craft_index)
		local fs = {
			...
			chat_craft.get_chat_craft_formspec(player, 12.25, scroll_index, craft_index),
		...
		context.page = "chat_craft:crafting"
	...
	sfinv.register_page("chat_craft:crafting", {
	    title = S("Fast Craft"),
	...
	chat_craft.regsiter_formspec_callback("", function(player, scroll_index, craft_index)
...
elseif gameid == "mineclone2" or minetest.get_modpath("mcl_formspec") then
	chat_craft.craft_sizes = {1, 4, 16, 64}
	chat_craft.grid_color = "#b9b9b9"
	chat_craft.highlight_color = "#84a19e"

	chat_craft.group_icons = {
		wood = "mcl_core:wood",
	...
	chat_craft.register_condition("crafting_table_neaby", {
	...
	chat_craft.register_condition("stonecutter_nearby", {
		func = function(player)
			local p_pos = player:get_pos():round()
		    p_pos.y = p_pos.y + 1
	...
	local function show_mcl_craft_page(player, scroll_index, craft_index)
		local fs = {
			...
			chat_craft.get_chat_craft_formspec(player, 11, scroll_index, craft_index),
			"box[" .. -2*p16 .. "," .. 6.25 - 2*p16 .. ";" .. 12.25 + 4*p16 .. "," .. 4.75 + 4*p16 .. ";#FFF0]",
		...
		fs = table.concat(fs)
		minetest.show_formspec(player:get_player_name(), "chat_craft:mcl_crafting", fs)
	end
	chat_craft.regsiter_formspec_callback("chat_craft:mcl_crafting", show_mcl_craft_page)
...
elseif gameid == "survivetest" then
	chat_craft.craft_sizes = {1, 5, 25, 99}
	chat_craft.grid_color = "#292929"
	chat_craft.highlight_color = "#54716e"
	...
	local function get_svt_sfinv_page(player, scroll_index, craft_index)
		local fs = {
			...
			chat_craft.get_chat_craft_formspec(player, 9.75, scroll_index, craft_index),
...
elseif gameid == "pmb_core" then
	...
	local function show_pmb_craft_page(player, scroll_index, craft_index)
		...
		minetest.show_formspec(player:get_player_name(), "chat_craft:pmb_crafting", fs)
	end
	chat_craft.regsiter_formspec_callback("chat_craft:pmb_crafting", show_pmb_craft_page)
	...
	pmb_inventory.register_formspec_process("inventory", "chat_craft:show_pmb_fc", function()
		local fc_button = {
			"style[show_pmb_fc;font=mono;border=false;bgimg_middle=12;padding=-12]",
			"style[show_pmb_fc;bgimg=pmb_button.png]",
			"style[show_pmb_fc:hovered;bgimg=pmb_button_hovered.png]",
	...
	minetest.log("warning", "[chat_craft] No integration method found for game: '" .. gameid .. "'. Use the chat command '/chat_craft' to access functionality")
end

if minetest.get_modpath("awards") then
	chat_craft.register_on_craft(function(player, itemstack, recipe)
