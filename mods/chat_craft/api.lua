...
function chat_craft.register_on_craft(func)
	table.insert(chat_craft.registered_on_crafts, func)
end
...
function chat_craft.simplify_recipe(recipe)
...
local recipes_by_rs = {}
function chat_craft.register_craft(recipe)
...
function chat_craft.register_condition(name, def)
	if chat_craft.registered_conditions[name] then
		minetest.log("warning", "[chat_craft] Crafting condition: '" .. name .. "' is being overrident")
	end
	def.name = name
	chat_craft.registered_conditions[name] = def
end
...
function chat_craft.inv_to_table(inv, list)
	local t = {}
...
function chat_craft.is_recipe_condition_fulfiled(recipe, player)
	for condition, _ in pairs(recipe.conditions) do
		if chat_craft.registered_conditions[condition].func(player) then
			return true
		end
	end
	return false
...
function chat_craft.get_craft_amount(recipe, inv)
	local n = 999
	for item, count in pairs(recipe.input) do
...
function chat_craft.get_craftable_recipes(player)
	local r = {}
	local inv = chat_craft.inv_to_table(player:get_inventory())

	for i, recipe in ipairs(registered_crafts) do
		if chat_craft.get_craft_amount(recipe, inv) > 0 and chat_craft.is_recipe_condition_fulfiled(recipe, player) then
			r[#r + 1] = i
		end
	end
...
function chat_craft.craft(player, recipe_index, amount)
